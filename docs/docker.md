...
Docker is optional. Stakl runs Compose v2-compatible commands through the `docker compose` CLI. The System page checks whether the executable and daemon are available. Docker errors include command output so a stopped daemon, invalid project, or missing image is distinguishable.
...
`compose_file`, project name, profile flags and env-file flags are passed as argument arrays. `docker.args` are extra **global Compose arguments**, before the subcommand. Compose env files and app `env_file` have different purposes: Compose consumes the former, while Stakl constructs the command's environment using the latter.
...
A Compose project already running when first detected is external. Stakl will not stop it automatically. Use Docker directly, or configure an explicit custom runner if adopting externally managed infrastructure is intentional. Projects started by Stakl retain ownership in SQLite across controller restarts.

Run `make docker-test` for a real disposable Compose integration check. It uses a unique `stakl-test-*` project, pulls `busybox:1.37` if needed, and removes only that test project afterward.
