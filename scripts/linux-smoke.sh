...
set -eu
mkdir -p /tmp/localdesk-smoke
cat > /tmp/localdesk-smoke/config.yml <<'YAML'
version: 1
server: {port: 49152, open_browser: false}
apps:
  worker:
    type: shell
    cwd: /tmp/localdesk-smoke
    start: {command: 'echo linux-ready; sleep 120'}
    health: {type: process, interval: 200ms}
    stop: {timeout: 1s}
YAML
export LOCALDESK_CONFIG=/tmp/localdesk-smoke/config.yml
binary=/binaries/localdesk-linux-arm64
"$binary" --no-browser >/tmp/localdesk-smoke/controller.log 2>&1 &
controller=$!
trap '"$binary" stop worker >/dev/null 2>&1 || true; kill "$controller" 2>/dev/null || true' EXIT
for i in $(seq 1 50); do [ -f /tmp/localdesk-smoke/instance.json ] && break; sleep .1; done
"$binary" start worker
